(() => {
    ...
    let _slideDown = (target, duration = 500, showmore = 0) => {
        if (!target.classList.contains("_slide")) {
            ...
            target.style.overflow = "hidden";
            let startHeight = showmore ? showmore : target.offsetHeight;
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const addAfterClass = () => {
            const items = document.querySelectorAll(".hero__highlights-item");
            if (items.length > 4) items.forEach(((item, index) => {
                if (index >= 2) item.classList.add("add-after");
            }));
        };
        addAfterClass();
        const container = document.querySelector(".hero__highlights");
        if (container) {
    ...
    function handleDynamicEllipsis() {
        const articles = document.querySelectorAll(".item-insights");
        articles.forEach((article => {
            const list = article.querySelector(".item-insights__list");
            const listItems = Array.from(list.querySelectorAll(".item-insights__list-item"));
            const existingEllipsis = list.querySelector(".item-insights__list-item.ellipsis");
            if (existingEllipsis) existingEllipsis.remove();
            listItems.forEach((listItem => {
                const link = listItem.querySelector("a, button");
                if (link) {
                    listItem.dataset.originalText = link.textContent;
                    link.textContent = listItem.dataset.originalText;
                    if (link.tagName === "BUTTON") {
                        const anchor = document.createElement("a");
                        anchor.href = "#";
                        anchor.className = link.className.replace("ellipsis__button", "").trim();
                        anchor.textContent = link.textContent;
                        listItem.replaceChild(anchor, link);
                    }
                }
                listItem.style.display = "inline-block";
            }));
            const availableWidth = list.offsetWidth;
            const ellipsisBuffer = 70;
            let totalWidth = 0;
            let lastVisibleIndex = -1;
            for (let i = 0; i < listItems.length; i++) {
                const itemWidth = listItems[i].offsetWidth;
                if (totalWidth + itemWidth <= availableWidth - ellipsisBuffer) {
                    totalWidth += itemWidth;
                    lastVisibleIndex = i;
                } else break;
            }
            if (lastVisibleIndex < listItems.length - 1) {
                const ellipsisItem = document.createElement("li");
                ellipsisItem.className = "item-insights__list-item ellipsis";
                const button = document.createElement("button");
                button.textContent = "...";
                button.className = "ellipsis__button";
                ellipsisItem.appendChild(button);
                listItems[lastVisibleIndex + 1].before(ellipsisItem);
                listItems.slice(lastVisibleIndex + 1).forEach((item => {
    ...
    window.addEventListener("resize", handleDynamicEllipsis);
    document.addEventListener("DOMContentLoaded", handleDynamicEllipsis);
    ...
    function documentActions(e) {
        ...
        if (targetElement.closest("[data-spoiler]")) {
            ...
            const parentElement = currentElement.closest(".faq__spoiler");
    ...
    if (spoilers.length) spoilers.forEach((spoiler => {
        spoiler.dataset.spoiler !== "open" ? spoiler.nextElementSibling.hidden = true : spoiler.classList.add("active");
    }));
    ...
    function updatePlaceholder() {
        ...
        if (window.innerWidth < 992) input.placeholder = "Email address"; else input.placeholder = "Enter email address";
    }
    updatePlaceholder();
